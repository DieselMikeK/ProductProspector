...
from product_prospector.core.shopify_collections import (
    default_collection_mapping_path,
    fetch_shopify_collection_mapping_table,
    save_collection_mapping_table,
)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Download Shopify collections and save only collection titles formatted like 'type | years'."
    )
    parser.add_argument(
        "--output",
        default="",
        help="Output .csv/.xlsx path. Default: app/required/mappings/ShopifyCollections.csv",
    )
    parser.add_argument(
        "--max-pages",
        type=int,
        default=120,
        help="Maximum GraphQL pages to read (250 collections per page).",
    )
    ...
    output_path = (
        ...
        if args.output
        else default_collection_mapping_path(required_root=required_root)
    )
    if output_path.suffix.lower() not in {".csv", ".xlsx"}:
        output_path = output_path.with_suffix(".csv")

    def on_progress(page: int, collections_seen: int, rows_matched: int) -> None:
        print(
            f"Reading Shopify collections... page {page}, "
            f"collections scanned {collections_seen}, formatted matches {rows_matched}"
        )

    table, fetch_error = fetch_shopify_collection_mapping_table(
        config=config,
    ...
    count, save_error = save_collection_mapping_table(table, output_path=output_path)
    if save_error:
        print(f"ERROR: Could not save mapping file: {save_error}")
        return 1

    print(f"Done. Saved {count} collection mapping row(s) to:")
