...
from product_prospector.core.shopify_brand_metaobjects import (
    default_brand_metaobject_path,
    fetch_brand_metaobjects,
    save_brand_metaobjects_table,
)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Download Shopify brand metaobjects into a local mapping file.")
    parser.add_argument(
        "--output",
        default="",
        help="Output .csv/.xlsx path. Default: app/required/mappings/ShopifyBrandMetaobjects.csv",
    )
    parser.add_argument(
        "--type",
        default="",
        help="Optional explicit metaobject type (example: brand). If omitted, auto-detect is used.",
    )
    ...
    output_path = Path(args.output).expanduser().resolve() if args.output else default_brand_metaobject_path(required_root)
    if output_path.suffix.lower() not in {".csv", ".xlsx"}:
        output_path = output_path.with_suffix(".csv")

    def on_progress(page: int, count: int) -> None:
        print(f"Reading Shopify brand metaobjects... page {page}, rows {count}")

    table, warning_or_note, resolved_type = fetch_brand_metaobjects(
        config=config,
    ...
    count, save_error = save_brand_metaobjects_table(table, output_path=output_path)
    if save_error:
        print(f"ERROR: Could not save mapping file: {save_error}")
        return 1

    print(f"Done. Saved {count} brand metaobject row(s) to:")
